...
class Display():
    ...
    def __init__(self, df_dict, df_list):
        self.dict = df_dict
        self.list = df_list
        ...
        self.attribut_widget = widgets.SelectMultiple(
            options=list(eval(self.dict[self.keys.value]).columns))

    def update_attribut(self, *args):
        self.attribut_widget.options = list(eval(self.dict[self.keys.value]).columns)

    def show_query(self, selected_df, attribut):
        df_name = self.dict[selected_df]
        indice = int(df_name[8])
        df = self.list[indice]
        return df[list(attribut)]

    def interact(self):
        self.keys.observe(self.update_attribut, names='value')
